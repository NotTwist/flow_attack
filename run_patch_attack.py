...
from models.model_utils import load_mde_model
...
def main():
    ...
    attack = get_attack(args.attack_type, model, mde_model=mde_model, mde_target=args.mde_target, num_steps=args.steps, no_softmax=args.no_softmax,
    ...
    for batch, (images, flow, valid, _) in enumerate(tqdm(data_loader)):
        ...
        metrics_tracker.update(original_flow, flow_pred,
                               gt_flow=flow, target_flow=of_target, inverse_flow=inverse_flow, valid=valid, tracked_flows=tracked_flows, original_depth=original_depth, attacked_depth = depth_pred, target_depth=mde_target, tracked_depths = tracked_depths)
