...
def colorplot_light_clipped(flow, auto_scale=True, max_scale=-1, return_max=False):
    """
    Visualizes optical flow with colors clipped to [0, 255] without normalizing.
    
    Args:
        flow (np.ndarray): Flow UV image of shape [H, W, 2].
        auto_scale (bool): If True, normalize flow by its max magnitude.
        max_scale (float): If > 0 and auto_scale is False, scales flow by this value.
        return_max (bool): If True, return the max magnitude along with the image.
    
    Returns:
        np.ndarray: Flow visualization image of shape [H, W, 3].
        Optional[float]: Maximum flow magnitude if return_max is True.
    """
    ...
    max_rad = rad.max() if auto_scale else max_scale if max_scale > 0 else rad.max()
    max_rad = max(max_rad, 1e-5)  # Avoid division by zero

    # Keep raw flow values but clip magnitudes
    if auto_scale or max_scale > 0:
        u = np.clip(u / max_rad, -1, 1)
        v = np.clip(v / max_rad, -1, 1)

    # Initialize flow image
    flow_image = np.zeros((u.shape[0], u.shape[1], 3), np.uint8)
    colorwheel = get_Middlebury_colorwheel()  # shape [55x3]
    ncols = colorwheel.shape[0]

    # Compute the angle and assign colors
    ...
    for i in range(3):
        ...
        col = (1 - f) * col0 + f * col1
        idx = rad <= 1
        col[idx] = 1 - rad[idx] * (1 - col[idx])  # Inner circle
        col[~idx] = col[~idx] * 0.75  # Outer circle
        flow_image[:, :, i] = np.clip(255 * col, 0, 255).astype(np.uint8)
        flow_image[nan, i] = 0  # Handle NaNs

    if return_max:
        return flow_image, max_rad
    else:
        return flow_image
