import torch
import torch.nn.functional as F
import random
import numpy as np
from typing import Literal
from .attack_base import OpticalFlowAttack
from models.model_utils import compute_flow
from cospgd import functions
from utils.process_images import get_image_tensors, get_image_grads, get_flow_tensors, replace_images_dic



###########################################
# NI-FGSM: Nesterov Iterative FGSM
###########################################

class NIFGSMOpticalFlowAttack(OpticalFlowAttack):
    """
    NI-FGSM реализует «lookahead»-шаг: перед вычислением градиента текущие изображения смещаются 
    в направлении накопленного градиентного вектора (momentum).
    """

    def __init__(self, model, target: Literal['zero', 'neg_flow', 'untargeted'],
                 epsilon=0.03, alpha=0.01, decay=1.0, device=None,
                 num_steps=20, common_perturb=False, clipping=True,
                 image_min=0, image_max=1, save_iterations: list = []):
        super().__init__(model, epsilon, alpha, device, target=target, learned=False)
        self.num_steps = num_steps
        self.decay = decay
        self.common_perturb = common_perturb
        self.clipping = clipping
        self.image_max = image_max
        self.image_min = image_min
        self.save_iterations = save_iterations

    def attack(self, inputs: torch.Tensor):
        orig_images = get_image_tensors(inputs, clone=True)
        inputs['images'].requires_grad_(True)
        momentum = torch.zeros_like(orig_images).to(self.device)
        flow_pred = self.model(inputs)['flows'].squeeze(0)
        target = self.target(flow_pred).to(self.device)
        target.requires_grad = False
        tracked_flows = {}

        for step in range(1, self.num_steps + 1):
            # Lookahead: смещаем изображение по текущему momentum
            lookahead_images = get_image_tensors(
                inputs, clone=True) + self.alpha * self.decay * momentum
            temp_inputs = inputs.copy()
            temp_inputs = replace_images_dic(temp_inputs, lookahead_images)

            temp_inputs['images'].requires_grad_(True)
            temp_inputs['images'].retain_grad()
            flow_pred = self.model(temp_inputs)['flows'].squeeze(0)
            loss = self.loss(flow_pred, target)
            self.model.zero_grad()
            loss.backward()
            grads = get_image_grads(temp_inputs)
            grad_norm = torch.norm(grads, p=1) + 1e-8
            momentum = self.decay * momentum + grads / grad_norm

            images = get_image_tensors(inputs)
            images = functions.step_inf(
                perturbed_image=images,
                epsilon=self.epsilon,
                data_grad=momentum,
                orig_image=orig_images,
                alpha=self.alpha,
                targeted=True,
                clamp_min=self.image_min,
                clamp_max=self.image_max,
                grad_scale=None,
            )
            inputs = replace_images_dic(inputs, images)
            inputs['images'].requires_grad_(True)

            if step in self.save_iterations:
                flow_pred = self.model(inputs)['flows'].squeeze(0)
                tracked_flows[step] = flow_pred.clone().detach()
        return {"final_images": inputs, "tracked_flows": tracked_flows}
