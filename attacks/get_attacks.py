...
def get_attack(attack_name, model, target: Literal['zero', 'neg_flow', 'untargeted'], epsilon=0.005, num_steps=20, no_softmax=True, save_iterations: list = []):
    """
    ...
    """
    if attack_name == "FGSM":
        return FGSMOpticalFlowAttack(model=model, epsilon=epsilon, num_steps=num_steps, target=target)

    elif attack_name == "PGD":
        return PGDOpticalFlowAttack(model=model, epsilon=epsilon, num_steps=num_steps, target=target)

    elif attack_name == "CosPGD":
        # Check if the attack requires 'no_softmax'
        return CosPGDOpticalFlowAttack(model=model, epsilon=epsilon, num_steps=num_steps, no_softmax=no_softmax, target=target, save_iterations=save_iterations)
    elif attack_name == 'GradCAM':
        return GradCAMOpticalFlowAttack(model=model, epsilon=epsilon, num_steps=num_steps, target=target)
    else:
