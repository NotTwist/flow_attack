...
class CustomLogger:
    ...
    def update(self, I1, I2, I1_p, I2_p, I1_attacked_def, I2_attacked_def, I1_unattacked_def, I2_unattacked_def,
        ...
        if True: #self.i==self.n-1 and not self.no_save:
            ...
            logging.save_flow(F_unattacked, self.i, self.output_folder, flow_name='flow_unattacked', auto_scale=True, max_scale=max_flow, unregistered_artifacts=self.unregistered_artifacts)
...
def evaluate_patch(args):
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name=folder_name, nested = nested) as run:
        ...
        A = PatchAdversary(args.patch_name,size=args.patch_size,angle=0,scale=1, change_of_variable=False).to(device)
