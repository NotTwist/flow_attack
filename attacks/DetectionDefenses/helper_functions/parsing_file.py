...
def create_parser(stage=None, attack_type=None):
    ...
    dataset_args.add_argument('--dataset', default='KittiRaw', choices=['Kitti15', 'Sintel', 'Spring', 'SintelSplitZhao', 'SintelSplitYang', 'SpringSplitScheurer', 'HD1KSplitScheurer', 'DrivingSample', 'KittiRaw', 'Test'],
    ...
    if attack_type in ["patch_attack_withdefense"]:
        ...
        patch_att_def_args.add_argument(
            "--patch_size", dest="patch_size", type=int, default=50, help="size of the patch")
        patch_att_def_args.add_argument(
            "--random_loc", action='store_false', help="Set position of patch on image to be random during training (default true)")
